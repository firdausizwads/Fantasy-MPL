...
function DraftBetaVisual({region}:{region:Region}){const blue=['FANNY','HARITH','KAJA'],red=['CHOU','BRUNO','FARAMIS'];const bans=['JOY','LING','CHIP','HAYABUSA'];return <section className="draftBetaVisual"><header><div><span>FUNCTIONAL CLOSED BETA PREVIEW</span><h2>Looks like a real draft room. Works like a test lab.</h2><p>Visitors can see the pick/ban flow, hero slots, side control and model panel before full official data is connected.</p></div><strong>INTERACTIVE BELOW</strong></header><div className="betaDraftScreen"><article className="blue"><small>BLUE SIDE</small>{blue.map(hero=><b key={hero}>{hero}</b>)}<em>FIRST PICK</em></article><div className="betaDraftCenter"><img src={REGION_INFO[region].logo} alt=""/><span>VS</span><small>BETA VISUAL</small></div><article className="red"><small>RED SIDE</small>{red.map(hero=><b key={hero}>{hero}</b>)}<em>RESPONSE</em></article></div><footer><span>BANS</span>{bans.map(hero=><i key={hero}>{hero}</i>)}<p>Use the draft board below to prove the tool records picks, bans, undo, reset, side switching and draft report generation.</p></footer></section>}
