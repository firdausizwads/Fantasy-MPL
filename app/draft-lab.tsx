...
function DraftBetaVisual({region}:{region:Region}){const picks=['FANNY','HARITH','KAJA','BRUNO','FARAMIS'];const bans=['JOY','LING','CHIP','HAYABUSA','CHOU'];const HeroSlot=({hero,type,index}:{hero:string;type:'PICK'|'BAN';index:number})=><span className={`betaHeroSlot ${type.toLowerCase()}`}><small>{type} {index+1}</small><img src={heroPhoto(hero)} alt={`${hero} hero portrait`} loading="lazy" decoding="async"/><b>{hero}</b></span>;return <section className="draftBetaVisual compactDraftPreview"><header><div><span>FUNCTIONAL CLOSED BETA PREVIEW</span><h2>Simple pick and ban preview.</h2><p>A clean 5-pick and 5-ban sample so beta users understand the draft flow before using the interactive board below.</p></div><strong>{REGION_INFO[region].name}</strong></header><div className="betaPreviewRows"><article><div><span>PICK PREVIEW</span><b>5 HERO PICKS</b></div><section>{picks.map((hero,index)=><HeroSlot hero={hero} type="PICK" index={index} key={hero}/>)}</section></article><article><div><span>BAN PREVIEW</span><b>5 HERO BANS</b></div><section>{bans.map((hero,index)=><HeroSlot hero={hero} type="BAN" index={index} key={hero}/>)}</section></article></div><footer><p>Tap an empty active slot in the real draft board below to choose heroes, test bans, undo moves and open the AI recommendation panel.</p></footer></section>}
