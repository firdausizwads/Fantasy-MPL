...
function DraftBetaVisual({region}:{region:Region}){const blue=['FANNY','HARITH','KAJA'],red=['CHOU','BRUNO','FARAMIS'];const bans=['JOY','LING','CHIP','HAYABUSA'];const HeroChip=({hero}:{hero:string})=><b><img src={heroPhoto(hero)} alt={`${hero} hero portrait`} loading="lazy" decoding="async"/><span>{hero}</span></b>;return <section className="draftBetaVisual"><header><div><span>FUNCTIONAL CLOSED BETA PREVIEW</span><h2>Looks like a real draft room. Works like a test lab.</h2><p>Visitors can see the pick/ban flow, hero portraits, side control and AI recommendation panel before full official data is connected.</p></div><strong>INTERACTIVE BELOW</strong></header><div className="betaDraftScreen"><article className="blue"><small>BLUE SIDE</small>{blue.map(hero=><HeroChip hero={hero} key={hero}/>)}<em>FIRST PICK</em></article><div className="betaDraftCenter"><img src={REGION_INFO[region].logo} alt=""/><span>VS</span><small>BETA VISUAL</small></div><article className="red"><small>RED SIDE</small>{red.map(hero=><HeroChip hero={hero} key={hero}/>)}<em>RESPONSE</em></article></div><footer><span>BANS</span>{bans.map(hero=><i key={hero}><img src={heroPhoto(hero)} alt="" loading="lazy" decoding="async"/><b>{hero}</b></i>)}<p>Use the draft board below to prove the tool records picks, bans, undo, reset, side switching and draft report generation.</p></footer></section>}
