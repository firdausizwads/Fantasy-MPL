...
export default function DraftLab({ region, notify }: { region: Region; notify?: (message: string) => void }) {
  ...
  return <div className="page liveDraftLab">
    <section className="draftLabHero genericDraftHero"><div><span>CURRENT-PATCH MLBB DRAFT TOOL</span><h1>BUILD THE DRAFT.<br/><em>READ THE NEXT MOVE.</em></h1><p>CHOOSE WHICH SIDE ACTS FIRST, RECORD EACH BAN AND PICK, THEN REVIEW PRIORITY BANS, POTENTIAL OPPOSING PICKS AND COUNTER OPTIONS.</p><div><button className={mode === 'companion' ? 'active' : ''} onClick={() => setMode('companion')}>LIVE COMPANION</button><button className={mode === 'sandbox' ? 'active' : ''} onClick={() => setMode('sandbox')}>SANDBOX</button></div></div><div className="toolBrandCard"><img src="/brand/fantasy-mpl-emblem.png" alt=""/><div><small>FANTASY MPL</small><b>DRAFT INTELLIGENCE</b><span>CURRENT PATCH TOOL</span></div></div></section>
