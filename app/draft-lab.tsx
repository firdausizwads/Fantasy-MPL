...
function DraftBetaVisual(){const picks=['FANNY','HARITH','KAJA','BRUNO','FARAMIS'];const bans=['JOY','LING','CHIP','HAYABUSA','CHOU'];const HeroCard=({hero,type,index}:{hero:string;type:'PICK'|'BAN';index:number})=><article className={`finalPreviewHero ${type.toLowerCase()}`}><div className="finalPreviewPortrait"><img src={heroPhoto(hero)} alt={`${hero} hero portrait`} loading="lazy" decoding="async"/></div><div className="finalPreviewText"><small>{type} {index+1}</small><b>{hero}</b></div></article>;return <section className="draftBetaVisual finalDraftPreview"><header><div className="finalPreviewCopy"><span>FUNCTIONAL CLOSED BETA PREVIEW</span><h2>Clean 5-pick and 5-ban preview.</h2><div className="finalPreviewDescription"><p>Compact hero cards show the draft flow clearly.</p><p>Tap the board below to test picks, bans, undo, reset and AI Recommendation.</p></div></div></header><div className="finalPreviewGrid"><section><div className="finalPreviewTitle"><span>PICK LINEUP</span><b>5 HERO PICKS</b></div><div className="finalPreviewCards">{picks.map((hero,index)=><HeroCard hero={hero} type="PICK" index={index} key={hero}/>)}</div></section><section><div className="finalPreviewTitle"><span>BAN LIST</span><b>5 HERO BANS</b></div><div className="finalPreviewCards">{bans.map((hero,index)=><HeroCard hero={hero} type="BAN" index={index} key={hero}/>)}</div></section></div></section>}
