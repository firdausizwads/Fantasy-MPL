...
async function runSync(triggeredBy:string|null){
 const url=process.env.NEXT_PUBLIC_SUPABASE_URL,key=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY,serviceRoleKey=process.env.SUPABASE_SERVICE_ROLE_KEY,pandaToken=process.env.PANDASCORE_API_TOKEN,syncSecret=process.env.PANDASCORE_SYNC_SECRET;
 const missing=[!url&&'NEXT_PUBLIC_SUPABASE_URL',!key&&'NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY',!serviceRoleKey&&'SUPABASE_SERVICE_ROLE_KEY',!pandaToken&&'PANDASCORE_API_TOKEN',!syncSecret&&'PANDASCORE_SYNC_SECRET'].filter(Boolean);
 ...
 const client=createClient(url!,serviceRoleKey!,{auth:{persistSession:false,autoRefreshToken:false}});const{data,error}=await client.rpc('ingest_pandascore_fixture_batch',{raw_secret:syncSecret!,matches:normalized as unknown as Json});if(error)return NextResponse.json({error:error.message},{status:500});const autoApply=await Promise.all(LEAGUES.map(async league=>{const{data:applyData,error:applyError}=await client.rpc('apply_verified_pandascore_fixtures',{raw_secret:syncSecret!,target_region:league.region});return{region:league.region,result:applyData,error:applyError?.message||null}}));return NextResponse.json({ok:true,triggered_by:triggeredBy||'cron',raw_fetched:rawMatches.length,accepted:normalized.length,rejected,result:data,auto_apply:autoApply});
}

export async function POST(request:NextRequest){
 const url=process.env.NEXT_PUBLIC_SUPABASE_URL,key=process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY,serviceRoleKey=process.env.SUPABASE_SERVICE_ROLE_KEY,syncSecret=process.env.PANDASCORE_SYNC_SECRET;
 const auth=request.headers.get('authorization')||'';const jwt=auth.startsWith('Bearer ')?auth.slice(7):'';
 if(!jwt)return NextResponse.json({error:'Unauthorized'},{status:401});
 if(!url||!key||!serviceRoleKey||!syncSecret)return NextResponse.json({error:'Fixture sync server configuration is incomplete'},{status:503});
