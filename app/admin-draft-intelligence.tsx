...
export default function AdminDraftIntelligence({region,notify}:{region:Region;notify:(message:string)=>void}){
  ...
  return <div className="intelligenceConsole">
    ...
    {activeTab==='sources'&&<div className="sourceWorkspace">
      <section className="panel sourceLibrary"><div className="intelligenceSectionHead"><div><span>REGISTERED SOURCES</span><h2>Source library</h2><p>Only documented and approved sources may power public recommendations.</p></div><strong>{config.sources.length}</strong></div>{config.sources.length?<div className="sourceCards">{config.sources.map(source=><article key={source.id}><header><span className={source.approval_status}>{source.approval_status.toUpperCase()}</span>{source.primary_for_model&&<em>PRIMARY</em>}</header><h3>{source.name}</h3><a href={source.provider_url} target="_blank" rel="noreferrer">{source.provider_url}</a><dl><div><dt>Licence</dt><dd>{source.license_name||'Not recorded'}</dd></div><div><dt>Commercial use</dt><dd>{source.commercial_use_confirmed?'Confirmed':'Unconfirmed'}</dd></div></dl>{source.attribution_text&&<p>{source.attribution_text}</p>}</article>)}</div>:<div className="intelligenceEmpty"><span>○</span><h3>No sources registered</h3><p>Add a candidate source using the review form.</p></div>}</section>
      <form className="panel sourceReviewForm" onSubmit={saveSource}><div className="intelligenceSectionHead"><div><span>SOURCE REVIEW</span><h2>Register or update source</h2><p>Save unconfirmed providers as Pending.</p></div></div><div className="formGrid"><label>SOURCE NAME<input value={sourceName} onChange={e=>setSourceName(e.target.value)} placeholder="Provider name"/></label><label>PROVIDER URL<input value={providerUrl} onChange={e=>setProviderUrl(e.target.value)} placeholder="https://"/></label><label>TERMS URL<input value={termsUrl} onChange={e=>setTermsUrl(e.target.value)} placeholder="https://"/></label><label>LICENCE<input value={licenseName} onChange={e=>setLicenseName(e.target.value)} placeholder="Licence name"/></label><label className="wide">PUBLIC ATTRIBUTION<input value={attribution} onChange={e=>setAttribution(e.target.value)} placeholder="Required public credit"/></label><label className="wide">REVIEW NOTES<textarea value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Approval status, contact and restrictions"/></label><label>REVIEW STATUS<select value={approval} onChange={e=>setApproval(e.target.value)}><option value="pending">PENDING</option><option value="approved">APPROVED</option><option value="rejected">REJECTED</option><option value="suspended">SUSPENDED</option></select></label><div className="sourcePermissions"><label><input type="checkbox" checked={commercial} onChange={e=>setCommercial(e.target.checked)}/><span><b>Commercial use confirmed</b><small>Written permission or compatible terms recorded</small></span></label><label><input type="checkbox" checked={primary} onChange={e=>setPrimary(e.target.checked)}/><span><b>Primary model source</b><small>Use this provider for public recommendations</small></span></label></div></div><div className="formAction"><p>Do not approve a source based on assumptions. Keep it Pending until its rights are documented.</p><button className="primary" disabled={busy}>SAVE SOURCE REVIEW</button></div></form>
    </div>}
