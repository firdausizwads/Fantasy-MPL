...
export default function GuestEntry({initialRegion,onExplore,onAuth}:{initialRegion?:Region;onExplore:(region:Region)=>void;onAuth:(mode:AuthMode)=>void}){
  ...
  return <main className={`guestEntry guestStage-${stage} guestScene-${scene} ${transitioning?'guestTransitioning':''}`}>
    ...
    {stage==='welcome'&&<section className="guestCinematic" aria-live="polite">
      ...
      <article className={`cinematicIntro ${scene===0?'active':''}`} aria-hidden={scene!==0}><span>THE REGIONAL FANTASY EXPERIENCE</span><h1>WELCOME TO<br/><em>FANTASY MPL.</em></h1><p>Fantasy. Predictions. Draft intelligence. Community.</p><img src="/brand/fantasy-mpl-emblem-display.webp" alt=""/></article>
      ...
      <article className={`cinematicFinal ${scene===4?'active':''}`} aria-hidden={scene!==4}><div className="cinematicFinalMarks">{regionEntries.map(([code,region])=><span key={code}><img src={region.thumb} alt=""/><b>{code}</b></span>)}</div><img className="cinematicFinalBrand" src="/brand/fantasy-mpl-emblem-display.webp" alt=""/><span>THE ARENA IS OPEN</span><h2>THREE REGIONS.<br/>ONE FANTASY ARENA.</h2><p>Choose your battleground next.</p></article>
